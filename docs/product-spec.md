...
| Club page redesign (v3.4)   | Pixel-perfect re-skin to `club-reference.html` (epic #617): gradient hero + 8-stat strip (#618), Membership Trend chart with key-date verticals (Jul 1/Oct 1/Apr 1/Jun 30) + DCP Status panel with per-tier gaps (#619), Close-to-Distinguished callout (repositioned below the trend row) + DCP Goals Progress panel — progress bar + per-goal status grid (#620). Loyal-blue palette, `[data-theme='dark']` dark mode. Goal Achievement Timeline pending (#621). | 🚧 In progress |
