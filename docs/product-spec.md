...
| Club page redesign (v3.4)   | Pixel-perfect re-skin to `club-reference.html` (epic #617): gradient hero + 8-stat strip (#618), Membership Trend chart with key-date verticals (Jul 1/Oct 1/Apr 1/Jun 30) + DCP Status panel with per-tier gaps (#619). Loyal-blue palette, `[data-theme='dark']` dark mode. | 🚧 In progress |
...
| Close to Distinguished (≤ 4 members)        | `closeToDistinguished.ts`                                | Banner / chip both use shared threshold `CLOSE_TO_DISTINGUISHED_MAX_MEMBERS = 4`. Beyond 4, indicator hides — multi-month drives are not "close". (#433) |
